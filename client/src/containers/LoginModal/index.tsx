  ...
  return (
    <Modal showModal={showModal}>
      <div className={`flex justify-center items-center h-full`}>
        <div
          className={`bg-gray-800 w-5/6 md:w-1/2 lg:w-1/3 h-1/2 rounded-3xl text-white`}
          ref={loginModalRef}
        >
          <div className={`flex justify-center items-center flex-col`}>
            <div className={`pt-6 pb-3 text-2xl w-full text-center`}>
              {LOGIN}
              {noMatchShow && (
                <div className={`text-red-500 text-sm pt-1`}>{NO_MATCH}</div>
              )}
              {matchShow && (
                <div className={`text-green-500 text-sm pt-1`}>{MATCH}</div>
              )}
            </div>
            <div className={`self-start w-5/6 m-auto`}>
              <InputError
                ...
                inputClassName={`bg-transparent border-b-[1px] outline-none pl-0 text-white max-w-sm`}
                label={EMAIL}
