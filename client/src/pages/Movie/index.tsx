...
const { PREMIERE, TITLE, PRODUCTION_YEAR, GENRE, ACTORS, FORMAT, DIRECTION, TRAILER, RESERVE } =
...
const Movie: FC<MovieProps> = ({ userData }): ReactElement => {
    ...
    const [selectedDateOption, setSelectedDateOption] = useState<DateOption>({} as DateOption);
    ...
    const getMovieDateTimeOptions = useCallback(() => {
        ...
        movie.runningTimes.forEach((entry) => {
            ...
            const newTime = `${stringToDate.getHours()}:${
                stringToDate.getMinutes() < 10 ? "0" : ""
            }${stringToDate.getMinutes()}`;
            newDateOptions.push({
                date: newDate,
                time: newTime,
                hallId: entry.hallId,
    ...
    const getTimeOptionsForSelectedDate = () => {
        const date = dateOptions.filter((option) => selectedDateOption.date === option.date);
    ...
    const onChangeDate = (newDate: string) => {
        const newDateOption = dateOptions.filter((option) => option.date.trim() === newDate.trim())[0];
        if (newDateOption) {
    ...
    const onChangeTime = (newTime: string) => {
        const newTimeOption = dateOptions.filter((option) => option.time === newTime)[0];
    ...
    const onClickConfirm = () => {
        navigate(
            `/seats/${id}/${selectedDateOption.date.substring(
                selectedDateOption.date.indexOf(": ") + 1,
                selectedDateOption.date.length
            )}/${selectedDateOption.time}/${selectedDateOption.hallId}/${movie?.is3D ? "3D" : "normal"}/${movie?.title}`
        );
    };
    ...
    return (
        <div className="text-white pb-14">
            <NavbarContainer triggerLogin={triggerLogin} setTriggerLoginFalse={() => setTriggerLogin(false)} />
            {showTrailer && movie && movie?.trailerUrl && (
                <TrailerModal showModal={showTrailer} youtubeLink={movie?.trailerUrl} toggle={toggleTrailerModal} />
            )}
            <ReservationModal
            ...
            {movie && (
                <div className="md:px-20 lg:px-60 py-10">
                    ...
                    <div className="flex items-center mt-6 mx-10 sm:mx-0">
                        <Icon src={"/clock-solid.svg"} alt="clock-icon" className="w-4 h-4 mr-2 items-center" />
